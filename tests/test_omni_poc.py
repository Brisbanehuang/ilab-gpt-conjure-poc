...
class OmniPOCTests(TempDirMixin, TestCase):
    ...
    def test_share_config_defaults_do_not_reference_a_production_site(self) -> None:
        config = load_omni_poc_config(Path(self.create_temp_dir()), {})
        self.assertFalse(config.enabled)
        self.assertEqual(config.login_url, "")
        self.assertEqual(config.dashboard_url, "")
        self.assertIn("Brisbanehuang/ilab-gpt-conjure-poc", config.source_url)
        self.assertNotIn("brislouise.online", str(config))

    def test_enabled_share_config_requires_an_explicit_login_url(self) -> None:
        with self.assertRaisesRegex(RuntimeError, "OMNI_POC_LOGIN_URL"):
            load_omni_poc_config(
    ...
    def test_enabled_share_config_accepts_custom_sub2api_and_origins(self) -> None:
        config = load_omni_poc_config(
            Path(self.create_temp_dir()),
            {
                "OMNI_POC_MODE": "1",
                "OMNI_POC_SECRET_KEY": Fernet.generate_key().decode("ascii"),
                "OMNI_BASE_URL": "https://api.example.test/v1",
                "OMNI_POC_LOGIN_URL": "https://portal.example.test/image-generator",
                "OMNI_POC_DASHBOARD_URL": "https://portal.example.test/dashboard",
                "OMNI_POC_AUTH_ORIGINS": "https://admin.example.test",
            },
        )
        self.assertEqual(config.base_url, "https://api.example.test/v1")
        self.assertEqual(config.login_url, "https://portal.example.test/image-generator")
        self.assertEqual(config.dashboard_url, "https://portal.example.test/dashboard")
        self.assertIn("https://portal.example.test", config.auth_origins)
        self.assertIn("https://admin.example.test", config.auth_origins)
...
class OmniPOCRouteTests(TempDirMixin, TestCase):
    def test_health_reports_omni_poc_mode(self) -> None:
        ...
        self.assertEqual(payload["omni_poc"]["login_url"], "https://portal.example.test/image-generator")
        self.assertEqual(payload["omni_poc"]["dashboard_url"], "https://portal.example.test/dashboard")
...
class OmniPOCValidationEndpointTests(TempDirMixin, TestCase):
    ...
    def test_auth_exchange_cors_uses_configured_login_origin(self) -> None:
        app, _ = OmniPOCGenerationTests.create_poc_app(self)
        ...
        self.assertEqual(response.headers.get("access-control-allow-origin"), "https://portal.example.test")
