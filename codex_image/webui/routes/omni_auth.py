...
def _set_auth_cors(request: Request, response: Response, allowed_origins: frozenset[str]) -> None:
...
def register_omni_auth_routes(app: FastAPI, ctx: WebUIContext) -> None:
    ...
    async def exchange_options(request: Request, response: Response) -> Response:
        ...
        _set_auth_cors(request, response, getattr(config, "auth_origins", frozenset()))
